...
class TSGuessingQuestionMultiChoiceContaminationEvaluator:
    ...
    async def _evaluate_async(self, executor, benchmark_path: str, scenario_state, language: str) -> dict:
        ...
        with htrack_block("TS-Guessing (question-multichoice) contamination evaluation"):
            ...
            metrics = {}
            metrics["exact_match"] = (
                sum(1 for i in range(len(responses_list)) if responses_list[i] == answers_list[i]) / len(responses_list)
                if responses_list
                else 0.0
            )
                ...
                try:
                    ...
                    for i in range(len(responses_list)):
                        ...
                        try:
                            ...
                            hlog(
                                f"Error calculating ROUGE for instance {results[i]['id']}: {e_rouge_item}. Response: '{resp_str}', Answer: '{ans_str}'"
                            )
            ...
            try:
                ...
                for i, result in enumerate(translated_parts):
                    if isinstance(result, Exception):
                        hlog(
                            f"Translation failed for part {i+1} ('{parts[i]}'): {result}. Using original English text."
                        )
    ...
    def _build_prompt(self, example, part_one, part_two, part_three, part_four, part_five, part_six):
        ...
        try:
            ...
            if wrong_choice_index >= len(self.alphabet):
                hlog(
                    f"Warning: wrong_choice_index {wrong_choice_index} exceeds alphabet length {len(self.alphabet)}. Using modulo."
                )
            ...
            for i, choice_item in enumerate(choices):
                ...
                if i >= len(self.alphabet):
                    hlog(
                        f"Warning: choice index {i} for example {example.get('id', 'Unknown ID')} exceeds alphabet length. Skipping this choice in prompt."
                    )
                    continue
